...
public class OrderBoard {
    ...
    // Press 2 — fulfill the next item on customer 1's order, remove them if done
    public void fulfillFirst() {
        if (customers.isEmpty()) return;
        boolean done = customers.get(0).fulfillNext();
        if (done) customers.remove(0);
    }

    // Add a new customer mid-game (live update)
    public void addCustomer(String stall) {
        customers.add(new OrderList(1, stall));
    }
    ...
    public void draw(Graphics2D g2) {
        ...
        int cols = customers.size();
        int panelWidth = cols * COL_WIDTH + PADDING * 2;

        // Find max rows needed so background height fits the tallest column
        int maxItems = 0;
        for (OrderList o : customers) maxItems = Math.max(maxItems, o.items.size());
        ...
        for (int i = 0; i < cols; i++) {
            ...
            int colX = PANEL_X + PADDING + i * COL_WIDTH;

            // Column divider (skip first)
            if (i > 0) {
                g2.setColor(new Color(255, 255, 255, 40));
                g2.setStroke(new BasicStroke(1f));
                g2.drawLine(colX - PADDING / 2, PANEL_Y + 6, colX - PADDING / 2, PANEL_Y + panelHeight - 6);
            }

            // Customer header
            g2.setFont(new Font("Arial", Font.BOLD, 13));
            g2.setColor(new Color(255, 220, 80));
            g2.drawString("Customer " + (i + 1) + ":", colX, PANEL_Y + PADDING + 14);
