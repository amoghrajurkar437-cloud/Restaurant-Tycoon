...
public class RestockPanel {
    ...
    // All layout values scaled 1.5x from the previous version
    ...
    // Move selection up/down through the item list
    public void moveSelection(int dir) {
        if (typingMode) return;
        selectedIndex = (selectedIndex + dir + Inventory.ITEM_NAMES.length) % Inventory.ITEM_NAMES.length;
    }
    ...
    public void confirmTransfer() {
        ...
        int taken  = inventory.transferToPlayer(selectedIndex, amount);
        System.out.println("Got " + taken + "x " + Inventory.ITEM_NAMES[selectedIndex]);
        typedQty   = "";
        typingMode = false;
    }
    ...
    public void draw(Graphics2D g2) {
        ...
        int rows   = Inventory.ITEM_NAMES.length;
        ...
        for (int i = 0; i < rows; i++) {
            ...
            int qty = inventory.stallItems[i];
            ...
            String arrow = sel ? "> " : "  ";
            g2.drawString(arrow + Inventory.ITEM_NAMES[i] + "  x" + qty, panelX + PADDING, rowY);
        }
